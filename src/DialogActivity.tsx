...
const DialogActivity = ({ activity, isVisible, onHide, onSave, onChange, isEditMode, responsables }: DialogActivityProps) => {
	return (
		<div className={`shadow-lg transform transition-transform ${isVisible ? 'translate-x-0 w-[35vw] p-4' : 'translate-x-full w-[0vw]'}`}>
			{activity && (
				<>
					<div className="flex justify-between items-center mb-4">
						<h2 className="text-xl font-bold">{activity?.nombreActividad || 'Nueva Actividad'}</h2>
						<Button onClick={onHide} label="X" />
					</div>
					...
					<InputText
						...
						disabled={!isEditMode}
					/>
					{isEditMode && <Button label="Guardar Actividad" onClick={onSave} />}
				</>
			)}
		</div>
	)
